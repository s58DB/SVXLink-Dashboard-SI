...
function getEchoLinkConnectedStateFromLog() {
        ...
        $state = array("known" => false, "nodes" => array());
        ...
        if (preg_match('/EchoLink: single connected station =\s*(.+)$/', $line, $matches) ||
            preg_match('/EchoLink: multiple connected stations =\s*(.+)$/', $line, $matches)) {
                ...
                $state["nodes"] = array_values(array_filter(array_map('trim', $nodes), function($node) {
                        return $node !== "" && $node !== "-";
                }));
        }
...
function getEchoLinkConnectedNodesFromLog() {
        $state = getEchoLinkConnectedStateFromLog();
        return $state["nodes"];
}
